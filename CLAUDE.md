...
`admin.html` predates and does **not share code** with the game's desktop layout, but converged on an equivalent design: `.tabs` becomes a left sidebar column and `#screen-app` becomes a CSS grid (`@media (min-width: 900px)`) rather than a wrapping-flex tab row. This was built separately from the game's `.dock-desktop`/`dockOverlayToDevice` overlay-docking system — don't assume shared JS between the two repos beyond the RTDB schema and `googleLinks`.

### "Аккаунт" — cross-device login without Telegram
Same Firebase **Email Link** (passwordless) mechanism as the game, writing to the *same* `googleLinks/<uid>` table — one email attached from either the game or this panel logs into both. Attaching an email is only possible from inside a real Telegram session (the form is hidden otherwise, to prevent hijacking someone else's login); the attach link embeds `?linkTelegramId=<id>` so it resolves correctly even opened on a different device with no Telegram session at all. Two one-time Firebase Console settings this depends on: Authentication → Sign-in method → Email/Password → the separate "Email link (passwordless sign-in)" sub-toggle, and the serving domain listed under Authentication → Settings → Authorized domains — without either, this tab breaks (`auth/operation-not-allowed` / `auth/unauthorized-domain`) while everything else (Telegram login) keeps working.
